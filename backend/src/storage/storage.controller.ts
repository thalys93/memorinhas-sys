...
import { ADMIN_ROLES, SHOPKEEPER_ROLES, USER_ROLES } from 'src/enums/RoleGroups';
...
export class StorageController {
    ...
    @Get('cloudinary-signature/public')
    @ApiOperation({
        summary: 'Assinatura pública para personalização de kit',
    })
    @ApiQuery({ name: 'id', type: String })
    @ApiQuery({ name: 'displayName', type: String, required: false })
    async getPublicCloudinarySignature(
        @Query('id') id: string,
        @Query('displayName') displayName?: string,
    ) {
        // ponytail: público só Assets; trocar por guest-token se abuso
        return this.storageService.generateCloudinarySignature(
            id,
            UploadTemplates.Assets,
            displayName,
        );
    }
    ...
    @RolesDecorator(...USER_ROLES, ...SHOPKEEPER_ROLES, ...ADMIN_ROLES)
